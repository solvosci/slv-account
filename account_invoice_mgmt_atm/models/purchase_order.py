...
class PurchaseOrder(models.Model):
    ...
    def create_atm_account_move(self):
        ...
        invoice_id = self.env['account.move'].browse(result.get('res_id')).exists()
        if invoice_id:
            invoice_id.atm_invoice = True
            ...
            if self.amount_total < 1000:
                invoice_id.invoice_cash_rounding_id = self.env.ref('account_invoice_mgmt_atm.account_cash_rounding_default').id
        return result
