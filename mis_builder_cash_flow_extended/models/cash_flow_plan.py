...
class CashFlowPlan(models.Model):
    _name = 'cash.flow.plan'
    _description = 'Cash Flow Plan'
    _order = 'id desc'

    name = fields.Char(string='Plan\'s Name', required=True)
    partner_id = fields.Many2one('res.partner', string='Partner')
    account_id = fields.Many2one('account.account', string='Account', required=True)
    balance = fields.Float(string='Balance / Amount', required=True)
    category_id = fields.Many2one('cash.flow.forecast.category', string='Cash Flow Forecast Line Category')
    ...
    periodicity = fields.Selection([
        ...
        ('weeks', 'Weekly'),
        ('months', 'Monthly')
    ], string='Periodicity', required=True, default='months')
    ...
    forecast_line_ids = fields.One2many(
        ...
        'cash_flow_plan_id',
        string='Generated Cash Flow Forecast Lines'
    )
    forecast_line_count = fields.Integer(
        string='Cash Flow Forecast Line Count',
    ...
    def _compute_forecast_line_count(self):
        ...
        mapped = {r['cash_flow_plan_id'][0]: r['cash_flow_plan_id_count'] for r in counts}
        for plan in self:
            ...
            return {
                ...
                'params': {
                    ...
                    'message': _(
                        ...
                        'Reduce the date range or increase the interval to generate all lines.',
    ...
    def action_view_generated_forecasts(self):
        self.ensure_one()
        return {
