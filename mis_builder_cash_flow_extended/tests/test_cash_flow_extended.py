...
class TestCashFlowExtended(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.category_parent = cls.env["cash.flow.forecast.category"].create(
            {"name": "Parent"}
        )
        cls.category_child = cls.env["cash.flow.forecast.category"].create(
        ...
        cls.plan_monthly = cls.env["cash.flow.plan"].create(
            {
                "name": "Plan Monthly",
                "account_id": cls.account.id,
                "balance": 100,
        ...
        cls.plan_weekly = cls.env["cash.flow.plan"].create(
            {
                "name": "Plan Weekly",
                "account_id": cls.account.id,
                "balance": 100,
        ...
        cls.plan_days = cls.env["cash.flow.plan"].create(
            {
                "name": "Plan Days",
                "account_id": cls.account.id,
                "balance": 100,
        ...
        cls.plan_with_values = cls.env["cash.flow.plan"].create(
            {
                "name": "My Plan",
                "account_id": cls.account.id,
                "balance": 100,
        ...
        cls.plan_regenerate = cls.env["cash.flow.plan"].create(
            {
        ...
        cls.plan_limit = cls.env["cash.flow.plan"].create(
            {
                "name": "Plan Limit",
                "account_id": cls.account.id,
                "balance": 100,
    ...
    def test_plan_date_constrain(self):
        with self.assertRaises(ValidationError):
            self.env["cash.flow.plan"].create(
                {
                    "name": "Invalid Plan",
    ...
    def test_plan_generate_forecasts_monthly(self):
        self.plan_monthly.action_generate_forecasts()
    ...
    def test_plan_generate_forecasts_weekly(self):
        self.plan_weekly.action_generate_forecasts()
    ...
    def test_plan_generate_forecasts_days(self):
        self.plan_days.action_generate_forecasts()
    ...
    def test_plan_regenerate_removes_previous_lines(self):
        self.plan_regenerate.action_generate_forecasts()
